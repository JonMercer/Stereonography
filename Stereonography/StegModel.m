...
-(NSString*) encryptWithQuestion:(NSString*)question  andAnswer:(NSString*)answer andMessage:(NSString*)message andImageKey:(NSString *)imageKey {
  ...
  for (NSUInteger j = 0; j < height; j++) {
    for (NSUInteger i = 0; i < width; i++) {
      ...
      unsigned char rreducedQualityDensity = R(color) & 0xFD;
      unsigned char greducedQualityDensity = G(color) & 0xFD;
      unsigned char breducedQualityDensity = B(color) & 0xFD;
      unsigned char areducedQualityDensity = A(color) & 0xFD;
      *pixel = RGBAMake(rreducedQualityDensity & ((byteToWrite >> 6) & 0x03),
                        greducedQualityDensity & ((byteToWrite >> 4) & 0x03),
                        breducedQualityDensity & ((byteToWrite >> 2) & 0x03),
                        areducedQualityDensity & ((byteToWrite >> 0) & 0x03));
      bytesWritten++;
